...
class ProgressBar:
    def __init__(
        ...
        num_iterations: int,
        verbose: bool = True,
        separate_scalar_display: bool = True,
        display_name: str = "Scalars",
    ):
        if verbose:  # create a nice little progress bar
            progress_bar_format = (
                "{desc} {n_fmt:" + str(len(str(num_iterations))) + "}/{total_fmt}|{bar}|{elapsed}<{remaining}"
            )
    ...
    def __call__(self, num_update_steps: int = 1, **kwargs):
        if self.progress_bar is not None:
            formatted_scalars = {
                key: "{:.3e}".format(value[-1] if isinstance(value, list) else value) for key, value in kwargs.items()
            }
            description = (
                f"{self._display_name}: "
                + "".join([str(key) + "=" + value + ", " for key, value in formatted_scalars.items()])
            )[:-2]
