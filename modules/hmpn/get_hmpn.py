...
def get_hmpn(*,
    ...
    architecture = base_config.get("architecture", "mpn").lower()
    if architecture == "gat":
        from modules.hmpn.pyg_wrappers.gat import GATWrapper
        assert isinstance(in_node_features, int), (f"Expected homogeneous graph, i.e., "
                                                   f" in_node_features to be an int, but got {in_node_features}")
        base = GATWrapper(**params, node_name=node_name)
    elif architecture in ["mpn", "hmpn"]:
