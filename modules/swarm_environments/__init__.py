...
from modules.swarm_environments.mesh_refinement import MeshRefinement
from modules.swarm_environments.util.utility_functions import (
    list_of_dicts_from_dict_of_lists,
    merge_nested_dictionaries,
)


def create_environments(
    environment_config: Dict[Union[str, int], Any], seed: Optional[int] = None
) -> Tuple[AbstractSwarmEnvironment, List[AbstractSwarmEnvironment], List[AbstractSwarmEnvironment],]:
    """
    Build and return a single training environment, a list of evaluation and a list of final environments
    for the task specified in the config.
    ...
    clean_base_params = _get_clean_base_params(raw_environment_params, keys=["evaluation", "final"])

    # Extract evaluation and final parameters
    evaluation_environment_param_list = _extract_environment_param_list(
        raw_environment_params.get("evaluation"), base_params=clean_base_params
    )
    final_environment_param_list = _extract_environment_param_list(
        raw_environment_params.get("final"), base_params=clean_base_params
    )
    ...
    final_environments = [
        env_loader.create(environment_class_name, environment_config=environment_params, seed=123)
...
def _get_clean_base_params(environment_params: Dict[str, Any], keys: List[str]) -> Dict[str, Any]:
...
def _extract_environment_param_list(
    subenvironment_params: Dict[str, Any], base_params: Dict[str, Any]
) -> List[Dict[str, Any]]:
    ...
    if subenvironment_params is not None and len(subenvironment_params) > 0:
        ...
        extracted_param_list = [
            merge_nested_dictionaries(destination=base_params, source=param) for param in extracted_params
        ]
    else:
