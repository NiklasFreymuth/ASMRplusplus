...
from skfem import Basis, ElementTriP1, ElementVector, Mesh, asm, condense, solve
...
class LinearElasticity(AbstractFiniteElementProblem2D):
    def __init__(
        self,
        *,
        ...
        assert (
            0 <= self._relative_stress_weight <= 1
    ...
    def _set_pde(self) -> None:
        ...
        if self._fixed_displacement:
            ...
            displacement_direction = self._random_state.uniform(low=0, high=np.pi, size=1).item()
            # draw value in [0, pi] to ensure that the displacement stretches rather than compresses the mesh
            displacement_magnitude = self._random_state.uniform(
                low=self._boundary_displacement_magnitude_range[0],
        ...
        mesh_ = mesh.with_boundaries(  # Label boundaries
            {
                ...
                "right": lambda x: np.isclose(x[0], self._boundary[2], atol=1e-10, rtol=1e-10),
            }
        )
        basis = Basis(mesh_, self.element_vector)
        return basis
    ...
    def _calculate_solution(self, basis: Basis, cache: bool) -> np.array:
        ...
        stress_tensor = self.material_model(sym_grad(interpolated_displacement))
        ...
        stress_norm = np.sqrt(
            0.5 * ((s[0, 0] - s[1, 1]) ** 2 + (s[1, 1] - s[2, 2]) ** 2 + (s[2, 2] - s[0, 0]) ** 2 + 6.0 * s[0, 1] ** 2)
        )
    ...
    def element_features(self, mesh: Mesh, element_feature_names: List[str]) -> Optional[np.array]:
        ...
        if "x_displacement" in element_feature_names:
            features.append(
                np.repeat(
                    np.array(self._boundary_x_displacement),
        ...
        if "y_displacement" in element_feature_names:
            features.append(
                np.repeat(
                    np.array(self._boundary_y_displacement),
                    repeats=num_elements,
                    axis=0,
                )
            )
    ...
    def additional_plots_from_mesh(self, mesh: Mesh) -> Dict[str, go.Figure]:
        ...
        if self._displacement is not None:
            ...
            norm_displacement_plot = self._get_scalar_plot(
                mesh=deformed_mesh,
                scalars=mesh_displacement_norm,
            ...
            x_displacement_plot = self._get_scalar_plot(
                mesh=deformed_mesh,
                scalars=mesh_x_displacement,
            ...
            y_displacement_plot = self._get_scalar_plot(
                mesh=deformed_mesh,
                scalars=mesh_y_displacement,
