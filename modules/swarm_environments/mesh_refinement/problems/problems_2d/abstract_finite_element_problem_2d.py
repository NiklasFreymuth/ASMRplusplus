...
class AbstractFiniteElementProblem2D(AbstractFiniteElementProblem, ABC):
    ...
    def _assert_consistency(self) -> None:
        from modules.swarm_environments.mesh_refinement.domains.domains_2d import (
            AbstractDomain2D,
        )

        assert isinstance(
            self._domain, AbstractDomain2D
    ...
    def _get_scalar_plot(self, mesh: Mesh, scalars: np.ndarray, title: str = "Value") -> go.Figure:
        ...
        Args:
            mesh: The 2d scikit FEM mesh to use for the plot and the wireframe
        ...
        scalar_plot = go.Figure(data=traces, layout=layout)
        return scalar_plot
