...
from modules.swarm_environments.mesh_refinement.problems.load_functions.abstract_density import (
    AbstractDensity,
)
from modules.swarm_environments.mesh_refinement.problems.load_functions.target_function_util import (
    build_gmm,
)
        ...
        **kwargs,
        ...
        if not self.fixed_target:
            ...
            if self._gmm_sample_mode == "random":
                ...
                nth_root = int(np.power(self._num_components, 1 / self._dimension))  # dimension can be 2 or 3
                assert (
                    nth_root**self._dimension == self._num_components
                ), "Number of components must be a perfect nth power for stratified sampling"

                # Generate one dimension
                mean_one_dimension = np.linspace(self._mean_position_range[0], self._mean_position_range[1], nth_root)

                # Create the meshgrid and flatten for all dimensions
                mesh = np.meshgrid(*[mean_one_dimension] * self._dimension)
                flattened_mesh = [axis.flatten() for axis in mesh]

                # Stack and transpose
                means = np.vstack(flattened_mesh).T
                ...
                means = self._random_state.normal(
                    loc=0.5,
                    scale=self._gmm_noise_scale,
