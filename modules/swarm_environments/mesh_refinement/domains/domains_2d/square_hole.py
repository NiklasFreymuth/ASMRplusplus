...
class SquareHole(AbstractDomain2D):
    """
    ...
    def __init__(
        ...
        assert minimum_hole_size > 1.0e-2, (
            f"Can not produce holes with negative minimum size, " f"given {minimum_hole_size}"
        )
        maximum_hole_position = (
            self._mean_hole_position[0]
    ...
    def _get_initial_mesh(self) -> MeshTri1:
        ...
        if self.fixed_domain:
            ...
            hole_size = np.array([self._mean_hole_size, self._mean_hole_size])
        else:
            hole_position = self._mean_hole_position + self._random_state.uniform(
                low=-self.maximum_position_distortion,
                high=self.maximum_position_distortion,
                size=2,
            )
            hole_size = self._mean_hole_size + self._random_state.uniform(
                low=-self.maximum_size_distortion,
        ...
        initial_mesh = ExtendedMeshTri1.init_square_hole(
