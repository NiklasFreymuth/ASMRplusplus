...
class Bottleneck(AbstractDomain2D):
    """
    ...
    def _get_initial_mesh(self) -> MeshTri1:
        ...
        if self.fixed_domain:
            boundary_nodes = np.array(
                [
                    [0.0, 0.0 + self.maximum_distortion],
                    [0.0, 1.0],
                    [0.5, 0.6],
            ...
            boundary_nodes = np.array([[0.0, 0.0], [0.0, 1.0], [1.0, 1.0], [1.0, 0.0]])
            ...
            distortions = self._random_state.uniform(low=0, high=self.maximum_distortion, size=(2,))
            ...
            bottleneck_offsets = self._random_state.uniform(low=-0.2, high=0.2, size=(2,))
