...
class ConvexPolygon(AbstractDomain2D):
    """
    ...
    def _get_initial_mesh(self) -> MeshTri1:
        ...
        """
        boundary_nodes = self._mean_positions + self._random_state.uniform(
            low=-self.maximum_distortion,
            high=self.maximum_distortion,
            size=(self.num_boundary_nodes, 2),
        )

        initial_mesh = ExtendedMeshTri1.init_convex_polygon(
