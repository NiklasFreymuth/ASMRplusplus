...
from modules.swarm_environments.mesh_refinement.problems import get_finite_element_problem_class
...
def _filter_included_fields(dictionary: Dict[Union[str, int], Any]) -> List[str]:
    """
...
class FEMProblemCircularQueue:
    def __init__(
        ...
        *,
        ...
        self._fem_problem_class: Type[AbstractFiniteElementProblem] = get_finite_element_problem_class(
            fem_config=fem_config
        )

        pde_config = fem_config.get(fem_config.get("pde_type"))
        self._pde_features = {
            "element_features": _filter_included_fields(pde_config.get("element_features", {})),
        }
    ...
    def mesh_dimension(self):
        return self.fem_config.get("domain", {}).get("dimension", 2)
