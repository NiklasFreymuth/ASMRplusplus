...
class VDGNGAT(AbstractMessagePassingBase):
    def __init__(
        ...
        *,
        ...
        assert (
            flip_edges_for_nodes is False
        ), f"Expected flip_edges_for_nodes to be False got {type(flip_edges_for_nodes)}"

        if isinstance(scatter_reduce_strs, list):
            assert len(scatter_reduce_strs) == 1, (
                f"Expected scatter_reduce_strs to be a list of length 1, " f"got {scatter_reduce_strs}"
            )
            scatter_reduce_strs = scatter_reduce_strs[0]
        assert scatter_reduce_strs in ["sum", "mean"], (
            f"Expected scatter_reduce_strs to be 'sum' or 'mean', " f"got {scatter_reduce_strs}"
        )
        self._node_name = node_name
        self.vdgn_gat_steps = nn.ModuleList(
            [VDGNGATStep(latent_dimension=latent_dimension) for _ in range(stack_config["num_steps"])]
        )
