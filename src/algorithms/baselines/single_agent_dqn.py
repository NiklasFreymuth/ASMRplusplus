...
from src.algorithms.baselines.architectures.single_agent_dqn_policy import (
    SingleAgentDQNPolicy,
)
...
class SingleAgentDQN(SwarmDQN):
    def _get_random_actions(self):
        ...
        actions = torch.randint(low=0, high=self._environment.num_agents, size=(1,)).to(self.device)
        return actions

    def policy_step(
        self, *, observation: InputBatch, deterministic: bool = True, no_grad: bool = False, **kwargs
