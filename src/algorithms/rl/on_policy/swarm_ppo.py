...
from src.algorithms.rl.normalizers.dummy_swarm_environment_normalizer import (
    DummySwarmEnvironmentNormalizer,
)
from src.algorithms.rl.normalizers.on_policy.swarm_environment_ppo_normalizer import (
    SwarmEnvironmentPPONormalizer,
)
...
from src.algorithms.rl.on_policy.buffers.get_on_policy_buffer import (
    get_on_policy_buffer,
)
from src.algorithms.rl.on_policy.util.ppo_util import (
    get_entropy_loss,
    get_policy_loss,
    get_value_loss,
)
...
class SwarmPPO(AbstractRLAlgorithm):
    ...
    def __init__(self, config: ConfigDict, seed: Optional[int] = None) -> None:
        ...
        self._clip_range: float = ppo_config.get("clip_range")
        self._max_grad_norm: float = ppo_config.get("max_grad_norm")
        self._entropy_coefficient: float = ppo_config.get("entropy_coefficient")
        self._value_function_coefficient: float = ppo_config.get("value_function_coefficient")
        self._value_function_clip_range: float = ppo_config.get("value_function_clip_range")
        self._value_function_aggr: str = ppo_config.get("value_function_aggr")
        assert self._value_function_aggr in [
            "mean",
            "sum",
            "max",
            "agent",
            ...
            f"Value function aggregation must be any of 'mean', 'sum', 'max', 'agent', 'spatial',"
            f" given '{self._value_function_aggr}' instead"
        )
        ...
        if self.algorithm_config.get("checkpoint", {}).get("experiment_name") is not None:
            ...
            assert isinstance(
                checkpoint.architecture, SwarmPPOActorCritic
    ...
    def _build_normalizer(self, ppo_config):
        ...
        if normalize_rewards or normalize_observations:
            environment_normalizer = SwarmEnvironmentPPONormalizer(
    ...
    def _build_buffer(self, ppo_config):
        ...
        gae_lambda: float = ppo_config.get("gae_lambda")
        buffer_device = self.device if sample_buffer_on_gpu else torch.device("cpu")
        rollout_buffer: AbstractMultiAgentOnPolicyBuffer = get_on_policy_buffer(
            buffer_size=self._num_rollout_steps,
            gae_lambda=gae_lambda,
            discount_factor=self._discount_factor,
            value_function_aggr=self._value_function_aggr,
            device=buffer_device,
            mixed_return_config=self.algorithm_config.get("mixed_return", {}),
            projection_type=ppo_config.get("projection_type", "sum"),
            normalize_mappings=ppo_config.get("normalize_mappings", False),
        )
        return rollout_buffer

    def _build_policy(self, ppo_config: ConfigDict) -> SwarmPPOActorCritic:
        return SwarmPPOActorCritic(
    ...
    def collect_rollouts(self) -> ValueDict:
        ...
        progress_bar = ProgressBar(
            num_iterations=self._num_rollout_steps,
            verbose=self._verbose,
            separate_scalar_display=False,
            display_name="Policy Rollouts",
        )
        ...
        for step in range(self._num_rollout_steps):
            ...
            reward_info = add_to_dictionary(
                reward_info,
                new_scalars={key: np.sum(value) for key, value in additional_information.items()},
            )
            if done:
                final_reward_info = add_to_dictionary(
                    final_reward_info,
                    new_scalars={key: np.sum(value) for key, value in additional_information.items()},
                )
            progress_bar()
    ...
    def _policy_sample_step(self, current_observation) -> Tuple[ValueDict, bool, InputBatch]:
        ...
        if self._value_function_aggr == "spatial":
            ...
            if isinstance(self.rollout_buffer, MixedOnPolicyBuffer):
        ...
        next_observation, reward = self._environment_normalizer.update_and_normalize(
            observations=next_observation, reward=reward
        )
        if done and self._bootstrap_truncated_dones and additional_information.get(keys.IS_TRUNCATED, False):
    ...
    def _bootstrap_terminal_reward(self, next_observation: InputBatch, reward: torch.Tensor) -> torch.Tensor:
        ...
        if self._value_function_aggr == "agent":
    ...
    def training_step(self) -> ValueDict:
        ...
        progress_bar = ProgressBar(
            num_iterations=self._epochs_per_iteration,
            verbose=self._verbose,
            separate_scalar_display=False,
        )
    ...
    def _train_batch(self, rollout_data) -> Tuple[torch.Tensor, ValueDict]:
        ...
        values, log_probabilities, entropy = self._policy.evaluate_actions(observations=observations, actions=actions)
        values = values.squeeze(dim=-1)  # flattened list of one value, either per node or per graph

        # ratio between old and new policy probabilities of the action given the state.
        # This should be *exactly* one at the 1st batch, except for numerical issues/non-determinism with GNNs,
        # and should slowly move away from one over the course of this training iteration
        # The exact 1 does not hold when observations are normalized, as the normalization is a moving average
        ratio = torch.exp(log_probabilities - old_log_probabilities)
        if self._value_function_aggr in ["mean", "sum", "max"] and not len(ratio) == self._batch_size:
            ...
            if self._value_function_aggr in ["mean", "sum"]:
                # use scatter_mean here for both mean and sum aggr,
                # as we want the probability ratios of the graph to be the
                # normalized sum of the ratios of its agents.
                # I.e., if we have a lot of agents, that does not mean
                # that the graph probability ratio should be higher.
                # This would the case if we use scatter_sum instead
                ratio = scatter_mean(ratio, batch, dim=0)
                entropy = scatter_mean(entropy, batch, dim=0)
                log_probabilities = scatter_mean(log_probabilities, batch, dim=0)
            elif self._value_function_aggr == "max":
                from torch_scatter import scatter_max

                ratio, idxs = scatter_max(ratio, batch, dim=0)
                entropy = entropy[idxs]
                log_probabilities = log_probabilities[idxs]
        # calculate losses
        policy_loss = get_policy_loss(advantages=advantages, ratio=ratio, clip_range=self._clip_range)
        value_loss = self._value_function_coefficient * get_value_loss(
            returns=returns,
            values=values,
    ...
    @environment_normalizer.setter
    def environment_normalizer(self, environment_normalizer: AbstractEnvironmentNormalizer):
        self._environment_normalizer = environment_normalizer
