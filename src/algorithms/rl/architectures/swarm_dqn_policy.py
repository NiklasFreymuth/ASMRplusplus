...
from src.algorithms.rl.architectures.dqn_modules.discrete_graph_q_net import (
    DiscreteGraphQNet,
)
...
def project_q_values_to_previous_step(
    ...
    if normalize_mapping:
        ...
        projected_q_values = projected_q_values * projected_q_values.shape[0] / agent_mappings.shape[0]
...
class SwarmDQNPolicy(AbstractArchitecture):
    def __init__(
        ...
        self._projection_type = dqn_config.get("projection_type")
        self._normalize_mappings = dqn_config.get("normalize_mappings", False)
    ...
    def _initialize_q_network(self, dqn_config, environment, network_config):
        ...
        self.q_network = DiscreteGraphQNet(
    ...
    def sample_actions(self, observations: InputBatch, deterministic: bool = False) -> torch.Tensor:
        ...
        if deterministic:
            ...
            if self.exploration_method == "epsilon_greedy":
                ...
                random_actions = torch.randint(low=0, high=self._action_dimension, size=(num_agents,))
                ...
                actions = torch.multinomial(pseudo_probabilities, num_samples=1).squeeze(1)
            else:
    ...
    def _extract_replay_data(
        self, replay_data: DQNBufferSamples, prioritized_replay: bool = False
    ...
    def gradient_step(self, replay_data) -> Tuple[torch.Tensor, np.array]:
        ...
        (
            ...
            dones,
            weights,
        ) = self._extract_replay_data(replay_data, prioritized_replay)
        with torch.no_grad():
            ...
            projected_q_values = project_q_values_to_previous_step(
                next_q_values=next_q_values,
                agent_mappings=agent_mappings,
                aggregation_method=self._projection_type,
                normalize_mapping=self._normalize_mappings,
            )
    ...
    def exploration_rate(self) -> float:
        ...
        """
        rate = max(
            (self._num_exploration_decay_steps - self._step) / self._num_exploration_decay_steps,
            0,
        )
