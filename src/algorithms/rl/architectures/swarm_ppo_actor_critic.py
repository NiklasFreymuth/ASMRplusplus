...
from util.torch_util.torch_util import (
    orthogonal_initialization as orthogonal_initialization_function,
)
...
class SwarmPPOActorCritic(AbstractArchitecture):
    def __init__(
        ...
        """
        super(SwarmPPOActorCritic, self).__init__(use_gpu=use_gpu, network_config=network_config, ppo_config=ppo_config)
        self._agent_node_type = environment.agent_node_type

        orthogonal_initialization = ppo_config.get("orthogonal_initialization")
        self._value_function_aggr = ppo_config.get("value_function_aggr")

        self.share_base = network_config.get("share_base")
        if self.share_base:  # one shared base for value and policy
            self.graph_base: AbstractMessagePassingBase = get_swarm_base(
                graph_env=environment,
                network_config=network_config,
                device=self._gpu_device,
            )
        else:  # a base each for value and policy
            self.policy_base: AbstractMessagePassingBase = get_swarm_base(
                graph_env=environment,
                network_config=network_config,
                device=self._gpu_device,
            )
            self.value_base: AbstractMessagePassingBase = get_swarm_base(
                graph_env=environment,
                network_config=network_config,
                device=self._gpu_device,
            )
        ...
        if isinstance(environment.action_space, gym.spaces.Box):
            ...
            (
                self.action_out_embedding,
                self.log_std,
            ) = action_distribution.proba_distribution_net(
                latent_dim=self.policy_mlp.out_features,
                log_std_init=ppo_config.get("initial_log_std"),
            )
    ...
    def forward(
        ...
        value_function_aggr: Optional[str] = None,
        ...
        Args:
            ...
            value_function_aggr: [Optional] Scope of the value function. If None, will use self._value_function_aggr.
                May be either "mean" or "agent"

        Returns:
            -An action *per agent*,
            -A scalar *per graph* iff self.value_function_aggr is set to "mean"/"sum"/"max", and per agent otherwise
            - A log_probability of taking this action  *per agent*

        """
        observations = self.to_gpu(observations)
        distribution, values = self._get_values_and_distribution(observations, value_function_aggr=value_function_aggr)
        actions = distribution.get_actions(deterministic=deterministic)
        log_probability = distribution.log_prob(actions)
        return actions, values, log_probability

    def evaluate_actions(
        self, observations: InputBatch, actions: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
    ...
    def _get_values_and_distribution(
        self, observations: InputBatch, value_function_aggr: Optional[str] = None
        ...
        * using the value_mlp and an aggregation to get a value.
          This value may either be per *graph* ([self._]value_function_aggr=="mean"/"sum"/"max") or
          per *node* ([self._]value_function_aggr=="agent")
        ...
        if value_function_aggr is None:
            value_function_aggr = self._value_function_aggr

        # potentially aggregate over evaluations per node to get one evaluation per graph
        if value_function_aggr == "mean":
            from torch_scatter import scatter_mean

            values = scatter_mean(values, batch, dim=0)
        elif value_function_aggr == "sum":
            from torch_scatter import scatter_sum

            values = scatter_sum(values, batch, dim=0)
        elif value_function_aggr == "max":
            from torch_scatter import scatter_max

            values = scatter_max(values, batch, dim=0)[0]
        elif value_function_aggr in ["agent", "spatial"]:
            # keep one evaluation per node
            pass
        else:
            raise ValueError(f"Unknown value function scope '{value_function_aggr}'")
