...
from src.algorithms.rl.off_policy.buffers.abstract_off_policy_buffer import (
    AbstractBufferSamples,
    AbstractOffPolicyBuffer,
)
...
DQNTransition = Tuple[
    InputBatch,
    Tensor,
    Union[np.array, float, torch.Tensor],
    InputBatch,
    Union[np.array, bool, torch.Tensor],
    Optional[Union[np.array, torch.Tensor]],
]
...
class SwarmDQNBuffer(AbstractOffPolicyBuffer):
    ...
    def put(
        ...
        next_observation: InputBatch,
        done: Union[bool, np.array],
        agent_mapping: Optional[np.array] = None,
    ) -> None:
        """
