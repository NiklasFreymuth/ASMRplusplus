...
from src.algorithms.rl.normalizers.swarm_environment_observation_normalizer import (
    SwarmEnvironmentObservationNormalizer,
)
from src.algorithms.rl.off_policy.buffers.swarm_dqn_buffer import SwarmDQNBuffer
from src.algorithms.rl.off_policy.buffers.swarm_dqn_prioritized_buffer import (
    SwarmDQNPrioritizedBuffer,
)
from util.function import add_to_dictionary, prefix_keys, safe_max, safe_mean, safe_min
...
class SwarmDQN(AbstractRLAlgorithm):
    ...
    def __init__(self, config: ConfigDict, seed: Optional[int] = None) -> None:
        ...
        if self.algorithm_config.get("checkpoint", {}).get("experiment_name") is not None:
            ...
            assert isinstance(
                checkpoint.architecture, SwarmDQNPolicy
    ...
    def _build_buffer(self, dqn_config: ConfigDict) -> Union[SwarmDQNBuffer, SwarmDQNPrioritizedBuffer]:
        ...
        if self.prioritized_buffer:
            ...
            return SwarmDQNPrioritizedBuffer(
    ...
    def fit_iteration(self) -> ValueDict:
        ...
        progress_bar = ProgressBar(
            num_iterations=self._steps_per_iteration,
        ...
        scalars = (
            {"mean_" + key: safe_mean(value) for key, value in scalars.items()}
    ...
    def _collect_rollout_transition(
        self, action_sampling_strategy: str = "agent"
        ...
        (
            ...
            additional_information,
        ) = self._environment.step(action=detach(actions))
    ...
    def _cast_reward_and_done(
        self,
        done: Union[bool, np.array],
        reward: Union[float, np.array],
        previous_num_agents: int,
    ...
    def _get_random_actions(self):
        ...
        actions = torch.randint(
            ...
            size=(self._environment.num_agents,),
        ).to(self.device)
        return actions

    def _collect_learning_start_samples(self):
        progress_bar = ProgressBar(
            num_iterations=self._initial_replay_buffer_samples - self._replay_buffer.size,
            verbose=self._verbose,
            separate_scalar_display=False,
    ...
    def training_step(self) -> ValueDict:
        ...
        return {"loss": np.mean(losses), "agents_per_batch": np.mean(agents_per_batch)}

    def policy_step(
        self,
        *,
        observation: InputBatch,
    ...
    @property
    def environment_normalizer(
        self,
    ) -> Union[SwarmEnvironmentObservationNormalizer, DummySwarmEnvironmentNormalizer]:
    ...
    def policy_class(self):
        ...
        if use_mixed_return:
            from src.algorithms.rl.architectures.swarm_dqn_mixed_return_policy import (
                SwarmDQNMixedReturnPolicy,
            )

            return SwarmDQNMixedReturnPolicy
        else:
