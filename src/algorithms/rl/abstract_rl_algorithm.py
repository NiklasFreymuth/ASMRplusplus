...
from modules.swarm_environments import AbstractSwarmEnvironment, create_environments
...
from util.function import add_to_dictionary, get_from_nested_dict, prefix_keys
...
class AbstractRLAlgorithm(AbstractIterativeAlgorithm, abc.ABC):
    def __init__(self, config: ConfigDict, seed: Optional[int] = None):
        ...
        self._bootstrap_truncated_dones = self.algorithm_config.get("bootstrap_truncated_dones")
        ...
        (
            self._environment,
            self._evaluation_environments,
            self._final_environments,
        ) = create_environments(environment_config=config.get("environment"), seed=seed)
        ...
        for _ in range(self._num_evaluation_episodes):
            ...
            while not done:
                ...
                if first_episode:
                    (
                        current_traces,
                        current_additional_render_information,
                ...
                reward_info = add_to_dictionary(
                    reward_info,
                    {"delta_critic_values": np.mean(values) - np.mean(previous_values)},
                )
                previous_values = values
            ...
            mean_infos_of_environments.append(
                prefix_keys(
                    dictionary={key: safe_mean(value) for key, value in reward_info.items()},
                    prefix="mean",
                )
            )
            last_infos_of_environments.append(
                prefix_keys(
                    dictionary={key: float(value[-1]) for key, value in reward_info.items()},
                    prefix="last",
                )
            )
    ...
    def save_checkpoint(
        ...
        policy_save_path = self.policy.save(
    ...
    def load_from_checkpoint(self, checkpoint_config: ConfigDict) -> SwarmRLCheckpoint:
        ...
        architecture = AbstractArchitecture.load_from_path(
            state_dict_path=state_dict_file, environment=self.environment
        )
        # load environment normalizer if available. Load from same iteration as network
    ...
    def additional_plots(self, iteration: int) -> Dict[Key, go.Figure]:
        ...
        for environment_position, environment in enumerate(self._evaluation_environments):
            ...
            evaluation_environment_plots = environment.additional_plots(
                iteration=iteration, policy_step_function=self.policy_step
            )
            additional_plots = additional_plots | prefix_keys(
                evaluation_environment_plots,
    ...
    def get_final_values(self) -> ValueDict:
        ...
        """
        self.set_training_mode(False)
        evaluator = RLAlgorithmEvaluator(
            config=self.config,
