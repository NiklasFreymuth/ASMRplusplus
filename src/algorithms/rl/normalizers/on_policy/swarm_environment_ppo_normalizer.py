...
class SwarmEnvironmentPPONormalizer(SwarmEnvironmentObservationNormalizer):
    ...
    def update_and_normalize(self, observations: InputBatch, reward: float = None) -> Tuple[InputBatch, float]:
        """
        Normalize the given observations and the given reward with the current statistics. Normalizes nodes, edges,
        and the reward independently. Adapts the statistics if train==True.
        Args:
