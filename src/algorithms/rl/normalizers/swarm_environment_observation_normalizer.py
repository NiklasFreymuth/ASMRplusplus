...
class SwarmEnvironmentObservationNormalizer(AbstractEnvironmentNormalizer):
    def __init__(
        ...
        super().__init__()
        if normalize_nodes:
            self.node_normalizers = TorchRunningMeanStd(
                epsilon=epsilon, shape=(graph_environment.num_node_features,)
            )
        ...
        if normalize_edges:
            self.edge_normalizers = TorchRunningMeanStd(
                epsilon=epsilon, shape=(graph_environment.num_edge_features,)
            )
    ...
    def normalize_observations(self, observations: InputBatch) -> InputBatch:
        ...
        if self.node_normalizers is not None:
            observations.__setattr__(
                "x",
                self._normalize_observation(observation=observations.x, normalizer=self.node_normalizers),
            )
        if self.edge_normalizers is not None:
            observations.__setattr__(
                "edge_attr",
                self._normalize_observation(
                    observation=observations.edge_attr,
